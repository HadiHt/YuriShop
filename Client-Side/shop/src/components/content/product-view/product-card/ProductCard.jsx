...
const ProductItem = ({ product, type }) => {
  ...
  const deleteFromWishList = () => {
    Axios.delete(
      "http://localhost:5000/api/Users/" + Wishlist.wishListId + "/wish"
    )
      .then((res) => {
        setIsWishlist(false);
      })
    ...
    <div className="product_card">
      ...
      <div className="product_box">
        <div className="product_header">
          <h2>{product.name}</h2>
          <i
            onClick={isWishlist ? deleteFromWishList : addToWishList}
            className={isWishlist ? "red fa fa-heart" : "grey fa fa-heart"}
          ></i>
        </div>
        {type === "view" ? <span>{product.price}K L.L</span> : ""}
        <p>{product.category}</p>
        <p onClick={navigation} className="shopName">
          Shop: {SHop.username}
        </p>
        {type === "cart" ? <CartCardProduct /> : ""}
      </div>
